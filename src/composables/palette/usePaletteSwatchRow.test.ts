...
describe('usePaletteSwatchRow', () => {
  ...
  it('opens the picker seeded with the current color', () => {
    const { picker, pickerValue, openPicker } = setup(['#112233'])
    const click = vi.spyOn(picker.value!, 'click')
    openPicker(0, mouseEvent())
    expect(pickerValue.value).toBe('#112233')
    expect(click).toHaveBeenCalled()
  })

  it('falls back to white when the slot is empty', () => {
    const { pickerValue, openPicker } = setup([''])
    openPicker(0, mouseEvent())
    expect(pickerValue.value).toBe('#ffffff')
  })
